...
$table_prefix = 'phpbb_';
define('CUSTOM_FIELDS_TABLE',		$table_prefix . 'cust_top_field');
define('CUSTOM_FIELDS_DATA_TABLE',	$table_prefix . 'cust_top_field_data');
define('CUSTOM_FIELDS_LANG_TABLE',	$table_prefix . 'cust_top_field_lang');
define('CUSTOM_LANG_TABLE',		$table_prefix . 'cust_top_lang');
...
class main_module
{
	...
	function main($id, $mode)
	{
		...
		$this->page_title = 'ACP_CUSTOM_TOPIC_FIELDS';
		...
		$this->type_collection = $phpbb_container->get('reactiv.customtopicfields.type_collection');
		...
		switch ($action)
		{
			case 'delete':
				...
				if (confirm_box(true))
				{
					...
					$db->sql_query('DELETE FROM ' . CUSTOM_FIELDS_TABLE . " WHERE field_id = $field_id");
					$db->sql_query('DELETE FROM ' . CUSTOM_FIELDS_LANG_TABLE . " WHERE field_id = $field_id");
					$db->sql_query('DELETE FROM ' . CUSTOM_LANG_TABLE . " WHERE field_id = $field_id");

					$db_tools = $phpbb_container->get('dbal.tools');
					$db_tools->sql_column_remove(CUSTOM_FIELDS_DATA_TABLE, 'pf_' . $field_ident);
					...
					while ($row = $db->sql_fetchrow($result))
					{
						...
						{
							$sql = 'UPDATE ' . CUSTOM_FIELDS_TABLE . "
					...
					add_log('admin', 'LOG_TOPIC_FIELD_REMOVED', $field_ident);
					trigger_error($user->lang['REMOVED_TOPIC_FIELD'] . adm_back_link($this->u_action));
				}
				else
				{
					confirm_box(false, 'DELETE_TOPIC_FIELD', build_hidden_fields(array(
						'i'			=> $id,
						'mode'		=> $mode,
						'action'	=> $action,
						'field_id'	=> $field_id,
			...
			case 'activate':
				...
				add_log('admin', 'LOG_TOPIC_FIELD_ACTIVATE', $field_ident);
				...
				trigger_error($user->lang['custom_field_ACTIVATED'] . adm_back_link($this->u_action));
			...
			case 'deactivate':
				...
				add_log('admin', 'LOG_TOPIC_FIELD_DEACTIVATE', $field_ident);

				trigger_error($user->lang['custom_field_DEACTIVATED'] . adm_back_link($this->u_action));
			...
			case 'edit':
				...
				{
					$sql = 'SELECT l.*, f.*
						FROM ' . CUSTOM_LANG_TABLE . ' l, ' . CUSTOM_FIELDS_TABLE . ' f
					...
					$custom_field = $this->type_collection[$field_type];

					// Get language entries
					$sql = 'SELECT *
						FROM ' . CUSTOM_FIELDS_LANG_TABLE . '
				...
				else
				{
					...
					$custom_field = $this->type_collection[$field_type];
					$field_row = array_merge($custom_field->get_default_option_values(), array(
				...
				$exclude = array(
					1	=> array('field_ident', 'lang_name', 'lang_explain', 'field_required'),
					2	=> array('field_length', 'field_maxlen', 'field_minlen', 'field_validation', 'field_novalue', 'field_default_value'),
					3	=> array('l_lang_name', 'l_lang_explain', 'l_lang_default_value', 'l_lang_options')
//					1	=> array('field_ident', 'lang_name', 'lang_explain', 'field_option_none', 'field_show_on_reg', 'field_show_on_pm', 'field_show_on_vt', 'field_show_on_ml', 'field_required', 'field_show_novalue', 'field_hide', 'field_show_topic', 'field_no_view', 'field_is_contact', 'field_contact_desc', 'field_contact_url'),
//					2	=> array('field_length', 'field_maxlen', 'field_minlen', 'field_validation', 'field_novalue', 'field_default_value'),
//					3	=> array('l_lang_name', 'l_lang_explain', 'l_lang_default_value', 'l_lang_options')
				...
				*/
				...
				$options = $custom_field->prepare_options_form($exclude, $visibility_ary);
				...
				foreach ($exclude[2] as $key)
				{
					...
					$field_data = $cp->vars;
					$var = $custom_field->get_excluded_options($key, $action, $var, $field_data, 2);
					$cp->vars = $field_data;
				...
				foreach ($exclude[3] as $key)
				{
					...
					$field_data = $cp->vars;
					$var = $custom_field->get_excluded_options($key, $action, $var, $field_data, 3);
					$cp->vars = $field_data;
				}
				...
				if ($submit) //  && $step == 1
				{
					...
					$error = $custom_field->validate_options_on_submit($error, $cp->vars);
				...
				foreach ($exclude as $num => $key_ary)
				{
					...
					{
						$var = $custom_field->prepare_hidden_fields($step, $key, $action, $field_data);
						if ($var !== null)
						{
							$_new_key_ary[$key] = $custom_field->prepare_hidden_fields($step, $key, $action, $field_data);
						}
					}
					$cp->vars = $field_data;
				...
				if (!sizeof($error))
				{
					...
					{
						$this->save_TOPIC_FIELD($cp, $field_type, $action);
					}
				...
				switch ($step)
				{
					...
					case 1:
						$template_vars = array(
							...
							'FIELD_TYPE'		=> $custom_field->get_name(),
						...
						$field_data = $cp->vars;
						$custom_field->display_options($template_vars, $field_data);
						$cp->vars = $field_data;
					...
					case 2:
						...
						$options = $custom_field->get_options($this->lang_defs['iso'][$config['default_lang']], $cp->vars);
		...
		$s_select_type = '';
		foreach ($this->type_collection as $key => $custom_field)
		{
			$s_select_type .= '<option value="' . $key . '">' . $custom_field->get_name() . '</option>';
		}
	...
	*/
	function build_language_options(&$cp, $field_type, $action = 'create')
	{
		...
		$custom_field = $this->type_collection[$field_type];
		$options = $custom_field->get_language_options($cp->vars);
	...
	* Save Profile Field
	*/
	function save_TOPIC_FIELD(&$cp, $field_type, $action = 'create')
	{
		...
		* @var	string	field_type		Type of the field we are dealing with
		* @var	array	field_data		Array of data about the field
		* @var	array	custom_fields	Array of fields to be sent to the database
		* @since 3.1.6-RC1
		*/
		$vars = array(
			'action',
		...
		if ($action == 'create')
		{
			...
			$sql = 'INSERT INTO ' . CUSTOM_FIELDS_TABLE . ' ' . $db->sql_build_array('INSERT', $custom_fields);
			$db->sql_query($sql);
		...
		{
			$sql = 'UPDATE ' . CUSTOM_FIELDS_TABLE . '
				SET ' . $db->sql_build_array('UPDATE', $custom_fields) . "
				WHERE field_id = $field_id";
			$db->sql_query($sql);
		}
		...
		if ($action == 'create')
		{
			...
			$db_tools = $phpbb_container->get('dbal.tools');
			$db_tools->sql_column_add(CUSTOM_FIELDS_DATA_TABLE, $field_ident, array($custom_field->get_database_column_type(), null));
		}
		...
		if ($action == 'create')
		{
			...
			$profile_sql[] = 'INSERT INTO ' . CUSTOM_LANG_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
		}
		else
		{
			$this->update_insert(CUSTOM_LANG_TABLE, $sql_ary, array('field_id' => $field_id, 'lang_id' => $default_lang_id));
		}
		...
		if (is_array($cp->vars['l_lang_name']) && sizeof($cp->vars['l_lang_name']))
		{
			...
			{
				$sql = 'DELETE FROM ' . CUSTOM_LANG_TABLE . "
					WHERE field_id = $field_id
		...
		$cp->vars = $custom_field->get_language_options_input($cp->vars);
		...
		if ($cp->vars['lang_options'])
		{
			...
			if ($action != 'create')
			{
				$sql = 'DELETE FROM ' . CUSTOM_FIELDS_LANG_TABLE . "
					WHERE field_id = $field_id
			...
			foreach ($cp->vars['lang_options'] as $option_id => $value)
			{
				...
				if ($action == 'create')
				{
					...
					$profile_sql[] = 'INSERT INTO ' . CUSTOM_FIELDS_LANG_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
				}
				else
				{
					$this->update_insert(CUSTOM_FIELDS_LANG_TABLE, $sql_ary, array(
						'field_id'	=> $field_id,
		...
		foreach ($profile_lang as $sql)
		{
			if ($action == 'create')
			{
				$profile_sql[] = 'INSERT INTO ' . CUSTOM_LANG_TABLE . ' ' . $db->sql_build_array('INSERT', $sql);
			}
			else
			{
				$lang_id = $sql['lang_id'];
				...
				$this->update_insert(CUSTOM_LANG_TABLE, $sql, array('lang_id' => (int) $lang_id, 'field_id' => $field_id));
			}
		}
		...
		if (sizeof($profile_lang_fields))
		{
			foreach ($profile_lang_fields as $sql)
			{
				if ($action == 'create')
				{
					$profile_sql[] = 'INSERT INTO ' . CUSTOM_FIELDS_LANG_TABLE . ' ' . $db->sql_build_array('INSERT', $sql);
				}
				else
				{
					$lang_id = $sql['lang_id'];
					...
					$this->update_insert(CUSTOM_FIELDS_LANG_TABLE, $sql, array(
						'lang_id'	=> $lang_id,
						'field_id'	=> $field_id,
						'option_id'	=> $option_id)
					);
		...
		if ($action == 'edit')
		{
			add_log('admin', 'LOG_TOPIC_FIELD_EDIT', $cp->vars['field_ident'] . ':' . $cp->vars['lang_name']);
			trigger_error($user->lang['CHANGED_TOPIC_FIELD'] . adm_back_link($this->u_action));
		}
		else
		{
			add_log('admin', 'LOG_TOPIC_FIELD_CREATE', substr($field_ident, 3) . ':' . $cp->vars['lang_name']);
			trigger_error($user->lang['ADDED_TOPIC_FIELD'] . adm_back_link($this->u_action));
		}
	}
